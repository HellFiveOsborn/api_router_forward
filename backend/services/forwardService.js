      ...
      db.all(sql, [], (err, rows) => {
        try { // Adiciona try...catch abrangente
          ...
          const parsedRows = (rows || []).map(row => {
            ...
            try {
              // Parseia o método (pode ser string única ou array JSON)
              row.metodo = parseJsonConfig(row.metodo, [row.metodo || 'GET']);
              // Se após parsear ainda for string, converte para array
              if (typeof row.metodo === 'string') {
                row.metodo = [row.metodo];
              }

              row.headers_in_config = parseJsonConfig(row.headers_in_config, {});
              row.headers_out_config = parseJsonConfig(row.headers_out_config, {});

              // Define tipo de params baseado nos métodos
              const hasBodyMethod = Array.isArray(row.metodo) && row.metodo.some(m => ['POST', 'PUT', 'PATCH'].includes(m));
              row.params_config = parseJsonConfig(row.params_config, { type: hasBodyMethod ? 'body' : 'query' });
            } catch (parseError) {
  ...
  findBySlugAndPath: (reqPath) => {
    return new Promise((resolve, reject) => {
      ...
      db.all(sql, [], (err, rows) => {
        ...
        // 2. Iterar e tentar fazer match de forma robusta (aceita com/sem barra antes do wildcard)
        for (const row of rows) {
          // Monta o prefixo base: /{slug}{custom_route?}
          let basePattern = `/${row.slug}`;
          if (row.custom_route && row.custom_route.trim() !== '' && row.custom_route !== '/') {
            const customRoute = row.custom_route.startsWith('/') ? row.custom_route : '/' + row.custom_route;
            basePattern += customRoute;
          }
          // Remove barras finais duplicadas antes de adicionar o wildcard
          basePattern = basePattern.replace(/\/+$/, '');

          // Tenta ambos os padrões (com e sem barra antes do wildcard)
          const patternsToTry = [
            // Usa apenas sintaxe de chaves (v8): {provider}, {*wildcard}
            `${basePattern}{*wildcard}`,
            `${basePattern}/{*wildcard}`,
          ];
          ...
          for (const p of patternsToTry) {
            try {
              console.log(`[Forward Service] Tentando match para ${reqPath} com padrão '${p}' (ID: ${row.id})`);
              console.log(`[Forward Service] custom_route: ${row.custom_route}, url_destino: ${row.url_destino}, slug: ${row.slug}`);

              const matcher = match(p, { decode: decodeURIComponent, strict: false });
              const matchResult = matcher(reqPath);

              if (matchResult) {
                console.log(`[Forward Service] Match encontrado para ${reqPath} com padrão ${p} (ID: ${row.id})`);
                localMatch = matchResult;
                localPattern = p;
                break;
              }
            } catch (matchError) {
              console.error(`[Forward Service] Erro ao tentar match com padrão '${p}' para ID ${row.id}:`, matchError);
          ...
          // Ajusta params e calcula wildcard manualmente para garantir previsibilidade
          matchedConfig = row;
          extractedParams = (localMatch && localMatch.params) ? localMatch.params : {};

          // Extração determinística de provider e wildcard a partir do path real
          // Prefixo do slug
          const slugPrefix = `/${row.slug}`;
          let rest = reqPath.startsWith(slugPrefix) ? reqPath.slice(slugPrefix.length) : '';
          rest = rest.replace(/^\/+/, ''); // remove barras iniciais

          // Se custom_route possuir {provider} e ainda não foi extraído, capture o primeiro segmento
          if ((row.custom_route || '').includes('{provider}') && !extractedParams.provider) {
            const parts = rest.split('/');
            extractedParams.provider = parts.shift() || '';
            rest = parts.join('/');
          }

          // O restante vira wildcard (sem barras à esquerda)
          extractedParams.wildcard = (rest || '').replace(/^\/+/, '');

          break; // encontrou e ajustou params/wildcard
        }
        ...
        if (matchedConfig) {
          try {
            // Parseia o método (pode ser string única ou array JSON)
            matchedConfig.metodo = parseJsonConfig(matchedConfig.metodo, [matchedConfig.metodo || 'GET']);
            // Se após parsear ainda for string, converte para array
            if (typeof matchedConfig.metodo === 'string') {
              matchedConfig.metodo = [matchedConfig.metodo];
            }
            ...
            // Define tipo de params baseado nos métodos
            const hasBodyMethod = Array.isArray(matchedConfig.metodo) && matchedConfig.metodo.some(m => ['POST', 'PUT', 'PATCH'].includes(m));
            matchedConfig.params_config = parseJsonConfig(matchedConfig.params_config, { type: hasBodyMethod ? 'body' : 'query' });
          } catch (parseError) {
